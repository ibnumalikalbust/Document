...
class VPN extends MY_Controller
{
    ...
    {
        $koneksi_router = $this->router_process("koneksi","");
        ...
        if(!empty($this->idusernzm)){
            ...
            if($this->form_validation->run() === TRUE){
                ...
                if(!$action){
                    ...
                    $logs = $this->model->insert("log_activity_user",$data_log);
                    $data_user = $this->model->gd("data_order","username,nomor_order,port,password","id = '$id'","row");
                    if(!empty($data_user)){
    ...
    public function delete_vpn($p)
    {
        if(!empty($this->idusernzm)){
            ...
            if($delete_proses == "OK"){
                ...
                if(!$action_del){
                    $data_order = $this->model->gd("data_order","username","id = '$id'","row");
                    $data_log = [
                        "id_user" => $this->id_user,
                        "tanggal" => date("Y-m-d H:i:s"),
